...
$.fn.titleNav = function (options) {
	...
	// 标题元素和导航元素
	let elTitles = $(this);
	let elNav = params.nav;
	// 滚动容器
	let elContainer = params.container;
	// 导航固定定位的偏移高度
	let offsetTop = params.offsetTop;

	if (!elContainer.length) {
		return elTitles;
	}

	let isContainerWindow = (elContainer[0] == window);
	...
	// 偏移位置
	let defaultOffsetTop = elNav.offset().top;
	...
	elContainer.on('scroll', function () {
		let indexNav = 0;
		let rectTopContainer = isContainerWindow ? 0 : elContainer[0].getBoundingClientRect().top;
		// 目前滚动的高度
		...
		if (elContainer.height() + scrollTop >= scrollHeight - 1) {
			...
			elTitles.each(function (index) {
				let distanceToTop = this.getBoundingClientRect().top - rectTopContainer;
		...
		// 获取目前需要高亮的导航元素
		let elNavs = elNav.children();
		let elTargetNav = elNavs.eq(indexNav);
	...
	elNav.on('click', 'a', function (event) {
		...
		let target = $(this).data('target') || $(href);
		// 导航索引
		let indexNav = elNav.find('a').index($(this));
		if (!target.length) {
